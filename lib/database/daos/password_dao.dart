...
@DriftAccessor(tables: [PasswordTable, PasswordFieldTable, CategoryItemTable])
class PasswordDao extends DatabaseAccessor<AppDatabase>
    ...
    if (force) {
      return (await passwords.get()).map((password) {
        return password.copyWith(
          title: encryptionDB.decryptText(password.title),
          iconPath: encryptionDB.decryptText(password.iconPath),
          authKey:
              password.authKey != null
                  ? Value(encryptionDB.decryptText(password.authKey!))
                  : const Value.absent(),
        );
      }).toList();
    }
  ...
  Future<int> insertPassword(PasswordTableCompanion password) async {
    ...
    try {
      return await into(passwordTable).insert(
        password.copyWith(
          title: Value(encryptionDB.encryptText(password.title.value)),
          iconPath: Value(encryptionDB.encryptText(password.iconPath.value)),
          authKey:
              (password.authKey.value != null &&
                      password.authKey.value!.isNotEmpty)
                  ? Value(encryptionDB.encryptText(password.authKey.value!))
                  : Value.absent(),
        ),
      );
    } catch (e) {
    ...
    await batch((batch) {
      for (int i = 0; i < passwordFields.length; i++) {
        ...
        final adjustedCompanion = originalCompanion.copyWith(
          ...
          value: Value(encryptor.encryptText(originalCompanion.value.value)),
        );
  ...
  Future<List<PasswordFieldTableData>> getPasswordFields(int passwordId) async {
    ...
    return rows.map((field) {
      final encryptionDB = sl<DatabaseEncryptorService>();
      return field.copyWith(value: encryptionDB.decryptText(field.value));
    }).toList();
  }
    ...
    try {
      ...
      final updateId = await (update(passwordTable)
        ...
        PasswordTableCompanion(
          title: Value(encryptionDB.encryptText(password.title)),
          iconPath: Value(encryptionDB.encryptText(password.iconPath)),
      ...
      log(
        "fieldsToDelete: $fieldsToDelete, oldFieldIds: $oldFieldIds, updatedFieldIds: $updatedFieldIds",
      );
      ...
      await batch((batch) {
        for (var field in fields) {
          log(
            "field: $field, type: ${field.type}, exist:${oldFieldMap.containsKey(field.type.index)}",
          );
          if (oldFieldMap.containsKey(field.type.index)) {
            ...
            batch.update(
              ...
              PasswordFieldTableCompanion(
                ...
                value: Value(encryptionDB.encryptText(field.value)),
      ...
      PasswordTableCompanion(
        ...
        authKey: Value(
          params.authKey.isEmpty
              ? null
              : encryptionDB.encryptText(params.authKey),
        ),
        updatedAt: Value(DateTime.now()),
      ),
    );
  ...
  Future<int> deletePassword(int id, Directory cacheDir) async {
    ...
    if (password.iconPath.isImage) {
      log(
        "password.iconPath.isImage: ${password.iconPath.isImage}, ${password.iconPath}",
      );
  ...
  Future<int> deletePasswords() async {
    await delete(categoryItemTable).go(); // Delete all category items first
    await delete(passwordFieldTable).go(); // Delete all password fields first
    return delete(passwordTable).go(); // Then delete all passwords
  }
}
