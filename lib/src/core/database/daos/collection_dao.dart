...
class CollectionDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<int> insertCollection(CollectionTableCompanion collection) async {
    try {
      ...
      return await into(collectionTable).insert(
        collection.copyWith(
          name: Value(encryptor.encrypt(collection.name.value!)),
          iconPath: Value(encryptor.encrypt(collection.iconPath.value)),
  ...
  Future<(CollectionTableData, List<CollectionItemTableData>)> getCollection(
    ...
    final encryptor = sl<DatabaseEncryptorService>();
    final collection =
        await (select(collectionTable)
          ..where((tbl) => tbl.id.equals(id))).getSingle();

    final items =
        await (select(collectionItemTable)
          ..where((tbl) => tbl.collectionId.equals(id))).get();

    return (
      collection.copyWith(
        name: Value(encryptor.decrypt(collection.name!)),
        iconPath: encryptor.decrypt(collection.iconPath),
      ),
      items.toList(),
    );
  }
  ...
  getCollections(GetAllCollectionsParams params) async {
    try {
  ...
  Future<int> updateCollection(CollectionModel collection) async {
    final encryptor = sl<DatabaseEncryptorService>();
    final updateCollection = await (update(collectionTable)
      ..where((tbl) => tbl.id.equals(collection.id!))).write(
      CollectionTableCompanion(
        name: Value(encryptor.encrypt(collection.name)),
        iconPath: Value(encryptor.encrypt(collection.iconPath)),
  ...
  Future<int> deleteCollection(int id) async {
    ...
    return await (delete(collectionTable)
  ...
  Future<List<String>> getAllCollectionsImages() async {
    ...
    return collections
        .map((e) => sl<DatabaseEncryptorService>().decrypt(e.iconPath))
        .toList();
  }
}
