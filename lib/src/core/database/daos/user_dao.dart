...
class UserDao extends DatabaseAccessor<AppDatabase> with _$UserDaoMixin {
  ...
  Stream<ProgressModel> updateMasterKey({
    ...
    try {
      ...
      final collectionsCount = (await select(collectionTable).get()).length;
      final cardsCount = (await select(cardTable).get()).length;
      final walletsCount = (await select(walletTable).get()).length;

      final total =
          passwordsCount + collectionsCount + cardsCount + walletsCount;
      ...
      final collections = _updateCollectionsEncryptor(
    ...
    for (var password in passwords) {
      await (update(passwordTable)
        ...
        PasswordTableCompanion(
          title: Value(await newEncryptor.encrypt(await encryptor.decrypt(password.title))),
          iconPath: Value(
            await newEncryptor.encrypt(await encryptor.decrypt(password.iconPath)),
          ),
          authKey:
              password.authKey != null
                  ? Value(
                    await newEncryptor.encrypt(await encryptor.decrypt(password.authKey!)),
                  )
                  : Value.absent(),
        ),
      );
      ...
      for (var field in passwordFields) {
        await (update(passwordFieldTable)
          ...
          PasswordFieldTableCompanion(
            value: Value(await newEncryptor.encrypt(await encryptor.decrypt(field.value))),
          ),
        );
      }
  ...
  StreamUpdate _updateCollectionsEncryptor({
    required DatabaseEncryptorService newEncryptor,
  }) async* {
    final encryptor = sl<DatabaseEncryptorService>();
    ...
    for (var collection in collections) {
      await (update(collectionTable)
        ..where((tbl) => tbl.id.equals(collection.id))).write(
        CollectionTableCompanion(
          name: Value(
            await newEncryptor.encrypt(await encryptor.decrypt(collection.name!)),
          ),
          iconPath: Value(
            await newEncryptor.encrypt(await encryptor.decrypt(collection.iconPath)),
          ),
        ),
      );
  ...
  StreamUpdate _updateCardsEncryptor({
    ...
    for (var card in cards) {
      await (update(cardTable)..where((tbl) => tbl.id.equals(card.id))).write(
        CardTableCompanion(
          title: Value(await newEncryptor.encrypt(await encryptor.decrypt(card.title))),
          holderName: Value(
            await newEncryptor.encrypt(await encryptor.decrypt(card.holderName)),
          ),
          cvv: Value(await newEncryptor.encrypt(await encryptor.decrypt(card.cvv))),
          number: Value(await newEncryptor.encrypt(await encryptor.decrypt(card.number))),
          pin:
              card.pin != null
                  ? Value(await newEncryptor.encrypt(await encryptor.decrypt(card.pin!)))
                  : Value.absent(),
        ),
      );
        ...
        await (update(cardFieldTable)
          ...
          CardFieldTableCompanion(
            hintText:
                field.hintText != null
                    ? Value(
                      await newEncryptor.encrypt(await encryptor.decrypt(field.hintText!)),
                    )
                    : Value.absent(),
            value: Value(await newEncryptor.encrypt(await encryptor.decrypt(field.value))),
          ),
    ...
    for (var wallet in wallets) {
      await (update(walletTable)
        ...
        WalletTableCompanion(
          title: Value(await newEncryptor.encrypt(await encryptor.decrypt(wallet.title))),
          seedPhrase: Value(
            await newEncryptor.encrypt(await encryptor.decrypt(wallet.seedPhrase)),
          ),
        ),
      );
