    ...
    for (var password in passwords) {
      await (update(passwordTable)
        ...
        PasswordTableCompanion(
          title: Value(newEncryptor.encrypt(encryptor.decrypt(password.title))),
          iconPath: Value(
            newEncryptor.encrypt(encryptor.decrypt(password.iconPath)),
          ),
          authKey:
              password.authKey != null
                  ? Value(
                    newEncryptor.encrypt(encryptor.decrypt(password.authKey!)),
                  )
                  : Value.absent(),
        ),
      );
      ...
      for (var field in passwordFields) {
        await (update(passwordFieldTable)
          ...
          PasswordFieldTableCompanion(
            value: Value(newEncryptor.encrypt(encryptor.decrypt(field.value))),
          ),
        );
      }
    ...
    for (var collection in collections) {
      await (update(collectionTable)
        ...
        CollectionTableCompanion(
          name: Value(
            newEncryptor.encrypt(encryptor.decrypt(collection.name!)),
          ),
          iconPath: Value(
            newEncryptor.encrypt(encryptor.decrypt(collection.iconPath)),
          ),
        ),
      );
  ...
  StreamUpdate _updateCardsEncryptor({
    ...
    for (var card in cards) {
      await (update(cardTable)..where((tbl) => tbl.id.equals(card.id))).write(
        CardTableCompanion(
          title: Value(newEncryptor.encrypt(encryptor.decrypt(card.title))),
          holderName: Value(
            newEncryptor.encrypt(encryptor.decrypt(card.holderName)),
          ),
          cvv: Value(newEncryptor.encrypt(encryptor.decrypt(card.cvv))),
          number: Value(newEncryptor.encrypt(encryptor.decrypt(card.number))),
          pin:
              card.pin != null
                  ? Value(newEncryptor.encrypt(encryptor.decrypt(card.pin!)))
                  : Value.absent(),
        ),
      );
        ...
        await (update(cardFieldTable)
          ...
          CardFieldTableCompanion(
            hintText:
                field.hintText != null
                    ? Value(
                      newEncryptor.encrypt(encryptor.decrypt(field.hintText!)),
                    )
                    : Value.absent(),
            value: Value(newEncryptor.encrypt(encryptor.decrypt(field.value))),
          ),
    ...
    for (var wallet in wallets) {
      await (update(walletTable)
        ...
        WalletTableCompanion(
          title: Value(newEncryptor.encrypt(encryptor.decrypt(wallet.title))),
          seedPhrase: Value(
            newEncryptor.encrypt(encryptor.decrypt(wallet.seedPhrase)),
          ),
        ),
      );
