...
class CSVExporter {
  ...
  CSVExporter()
    : appDatabase = sl<AppDatabaseWrapper>(),
  ...
  Future<List<File>> _exportPasswords(String separator) async {
    final rawData = await appDatabase.db.passwordDao.getPasswords(
      force: true,
    ...
    final data = await Future.wait(
      rawData.map((password) async {
        ...
        final fields = await appDatabase.db.passwordDao.getPasswordFields(
          password.id,
        );
  ...
  Future<List<File>> _exportWallets(String separator) async {
    final data = await appDatabase.db.walletDao.getAllWallets(
      force: true,
  ...
  Future<List<File>> _exportCards(String separator) async {
    final data = await appDatabase.db.cardDao.getCards(
      force: true,
