...
class AuthLocalDatasourceImpl implements AuthLocalDatasource {
  ...
  Future<UserModel> register(UserModel userModel) async {
    ...
    final userId = await appDatabase.db.userDao.insertUser(
      UserTableCompanion(
    ...
    final userTableData = await appDatabase.db.userDao.getUserById(userId);
  ...
  Future<UserModel> login(String password) async {
    final userTableData = await appDatabase.db.userDao.getUserByPassword(password);
  ...
  Stream<ProgressModel> update(UpdateUserParams params) async* {
    final updateProgress = appDatabase.db.userDao.updateMasterKey(
      id: params.userId,
  ...
  Future<UserModel> updateHint(UpdatePasswordHintParams params) async {
    final updateId = await appDatabase.db.userDao.updateHint(
      passwordHint: params.hint,
    );
    if (updateId.$1 == 0) throw NotFoundFailure();

    final updatedUser = await appDatabase.db.userDao.getUserById(updateId.$1);
