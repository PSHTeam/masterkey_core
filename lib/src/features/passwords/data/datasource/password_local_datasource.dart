...
abstract class PasswordLocalDatasource {
  ...
  Future<PasswordModel> getPassword(int paramsId);
...
class PasswordLocalDatasourceImpl implements PasswordLocalDatasource {
  ...
  const PasswordLocalDatasourceImpl(this.appDatabase, this.cacheDir);

  @override
  Future<PasswordModel> addPassword(PasswordModel password) async {
    try {
      // log("$password", name: "PasswordLocalDatasourceImpl");
      final newId = await appDatabase.passwordDao.insertPassword(
  ...
  @override
  Future<PasswordModel> getPassword(int paramsId) async {
    return await getPasswordInfo(paramsId);
  }

  @override
  Future<PasswordModel> updatePassword(UpdatePasswordParams params) async {
    final updatedRows = await appDatabase.passwordDao.updatePassword(params);
  ...
  Future<bool> deletePassword(int passwordId) async {
    final response = await appDatabase.passwordDao.deletePassword(
      passwordId,
      cacheDir,
    );
  ...
  Future<List<PasswordModel>> getAllPasswords(
    ...
    try {
      final passwordlist = await appDatabase.passwordDao.getPasswords(params);
  ...
  Future<PasswordModel> toggle2FA(Toggle2FAParams params) async {
    // log(params.toString());
    final updateId = await appDatabase.passwordDao.toggle2FA(params);
  ...
  Future<PasswordModel> toggleArchiveStatus(
    ...
    final updateId = await appDatabase.passwordDao.toggleArchiveStatus(params);
    if (updateId == 0) throw UnknownFailure();
  ...
  Future<PasswordModel> getPasswordInfo(int paramsId) async {
    final newPassword = await appDatabase.passwordDao.getPasswordById(paramsId);
