...
abstract class WalletLocalDatasource {
  ...
  Future<WalletModel> getWallet(String params);
  Future<List<WalletModel>> searchWallet(SearchWalletParams params);
...
class WalletLocalDatasourceImpl implements WalletLocalDatasource {
  ...
  Future<List<WalletModel>> getAllWallets(GetAllWalletsParams params) async {
    ...
    // log('Wallet Table Data: $walletTableData');
    ...
    // log('Wallets: $allWallets');

    return allWallets;
  }

  @override
  Future<WalletModel> getWallet(String params) async {
    final walletTableData = await appDatabase.walletDao.getWalletById(
      params.toString() as int,
    );
    if (walletTableData == null) {
      throw WalletFailure();
    } else {
  ...
  @override
  Future<List<WalletModel>> searchWallet(SearchWalletParams params) async {
    final result = await appDatabase.walletDao.searchWallets(
      params.query,
      limit: params.limit,
    ...
    final wallets =
        result.map((wallet) => WalletModel.fromTableData(wallet)).toList();
