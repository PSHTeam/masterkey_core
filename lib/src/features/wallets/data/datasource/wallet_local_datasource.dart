...
class WalletLocalDatasourceImpl implements WalletLocalDatasource {
  ...
  Future<WalletModel> create(WalletParams wallet) async {
    final walletId = await appDatabase.db.walletDao.insertWallet(wallet);
    final walletTableData = await appDatabase.db.walletDao.getWalletById(walletId);
  ...
  Future<bool> delete(int params) async {
    final deleteId = await appDatabase.db.walletDao.deleteWallet(params);
    return deleteId != 0;
  }

  @override
  Future<List<WalletModel>> getAllWallets(GetAllWalletsParams params) async {
    final walletTableData = await appDatabase.db.walletDao.getAllWallets(params);
  ...
  Future<WalletModel> update(UpdateWalletParams params) async {
    final updateId = await appDatabase.db.walletDao.updateWallet(
      id: params.walletId,
    ...
    final walletTableData = await appDatabase.db.walletDao.getWalletById(
      params.walletId,
    );
  ...
  Future<WalletModel> toggleWalletArchiveStatus(
    ...
    final updateId = await appDatabase.db.walletDao.toggleArchiveStatus(
      params: params,
    );
